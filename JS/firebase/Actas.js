...
Actas.getPorvinciaByDepartamento = async (nombreDepartamento, ambito) => {
  const nombreColeccion =
    ambito == "PERU" ? "Participacion_N_Provincia" : "Participacion_Provincia";
  const sql = query(
...
Actas.getDistritoByProvincia = async (nombreProvincia, ambito) => {
  const nombreColeccion =
    ambito == "PERU" ? "DistritosNacionales" : "Distritos_Extranjeros";
...
Actas.getLocalesByDistrito = async (nombreDistrito, ambito = "PERU") => {
  const nombreColeccion =
    ambito == "PERU" ? "Locales_Nacionales" : "Locales_Extranjeros";
  const sql = query(
    collection(DBFirebase, nombreColeccion),
...
Actas.getGruposByLocal = async (idLocal, ambito = "PERU") => {
  const nombreColeccion =
    ambito == "PERU" ? "Grupos_Nacionales" : "Grupos_Extranjeros";
  const sql = query(
    collection(DBFirebase, nombreColeccion),
